...
class MetricsCalculator(object):
    ...
    def get_effective_queries_per_hour(self, queries_per_hour=None):
        ...
        olap_data = self.data[self.data['transactiontype']
                                    >= self.OLAP_QUERY_LOWER_ID]
            # since we are parsing old output format, we need a hack to 
        # determine number of workers
        num_workers = int(re.match(r'.*MIXED\d*_(\d*).raw',
                 self.data_path).group(1))
    ...
    def load_data(self):
        ...
        data.columns = ['transactiontype', 'starttime', 'latency']
